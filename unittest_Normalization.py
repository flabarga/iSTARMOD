...
class FITSObject(object):
    ...
    def evalReject (self, valueSpectrum, valueAdjustment, above,  reject_param):
        # if absortion_no_emission == True:
        #     continuum_shift = -0.01
        # else:
        #     continuum_shift = 0.01
        if (not above):
            return (valueSpectrum > valueAdjustment * reject_param)
        else:
            return ((valueSpectrum / valueAdjustment) < (2.04 - reject_param))      
            
          
    def continuum_det_and_normalization (self, rejt, strain, inputParams):
        # This fragment of code has been taken from ARESv2 of Sousa after being translated to python from C
        ...
        # FIRST RUN OF THE POINTS' REJECTION 
        vecx = []
        vecy = []
        nvec = 0
        ...
        for i in range(nx_elements2):
            ...
            if (self.evalReject(workingValues2[i], adjustValues3[i], True, rejt) and (i+1 < nx_elements2 and math.fabs(workingValues2[i]-workingValues2[i+1]) < strain * workingValues2[i])):
                ...
                nvec+=1
        print("nvec = ", nvec)
        #Once we have determined the relevant points to define the continuum, we must determine the continuum making a second adjustment
        ...
        # adjustValues2 has received the values of the non-rejected points
        # then it must be done the selection of points outside the emission zone
        # for i in range(nx_elements):
        #     if (self.workingLambdaValues[i] < inputParams.pixelExcl[1][0] or self.workingLambdaValues[i] > inputParams.pixelExcl[1][1]):
        #         adjustValues5.append(cpy.copy(adjustValues2[i]))
        
        # # SECOND RUN OF THE POINT'S REJECTION 
        # vecx2 = []
        # vecy2 = []
        # nvec = 0
        # nx_elements2 = len(adjustValues5)
        # for i in range(nx_elements2):
        #     #Thin out points belonging to spectral lines or noise. The remaining points supposedly belongs to the continuum
        #     if (self.evalReject(workingValues[i],adjustValues5[i],rejt) and (i+1 < nx_elements2 and math.fabs(workingValues[i] - workingValues[i+1]) < strain * workingValues[i])):
        #         vecx2.append(cpy.copy(workingLambdas[i]))
        #         vecy2.append(cpy.copy(workingValues[i]))
        #         nvec+=1
        # print("nvec = ", nvec)
        # 
        ...
        # poly_fitn_coefs = np.polyfit(vecx2,vecy2, 2)
        # adjustValues3    = np.polyval(poly_fitn_coefs,self.workingLambdaValues)
        
        
        # normalization by the just found continuum
        for i in range(nx_elements):
            if(i < nx_elements-1): 
                self.workingDataValues1[i] = self.workingDataValues1[i] / adjustValues5[i]
            else:
                self.workingDataValues1[i] = 1
            
        return vecx2, vecy2, adjustValues4, adjustValues5
...
def readFitsFile(inputFITS, order):
    ...
    if ".fits" in inputFITS:
        ...
        if numberOfAxis == 2 and ctype1 == "MULTISPE":
            ...
            return objectFITS
        elif numberOfAxis == 1 and (ctype1 == "LINEAR" or "Linear"):
            ...
            if objectFITS.carmenesType == 5 or objectFITS.carmenesType == 6: ########################ORIGINAL OR TELLURIC CORRECTED#######################
            ...
            elif objectFITS.carmenesType == 4: #####################STACKED#########################
...
def unittestNormalizeSpectra(spcfFile):
    ...
    inputObject = readFitsFile(inputParams.objectInputFits, inputParams.spectraFormatAperture)
    ...
    if (inputObject.SNR < 80):
        ...
        strain = .1
    vecx, vecy, workingData2, workingData3 = inputObject.continuum_det_and_normalization (rejt, strain, inputParams)
    plt.plot(inputObject.workingLambdaValues,inputObject.workingDataValues1, 'g' )
    # plt.plot(inputObject.workingLambdaValues,inputObject.workingDataValues3 )
    plt.plot(vecx,vecy, 'gx' )     
    plt.plot(inputObject.workingLambdaValues,workingData2, 'm--' )
    plt.plot(inputObject.workingLambdaValues,workingData3, 'b-.' )
    plt.axhline(y=1.0, linestyle='dashed')
    # plt.plot(inputObject.workingLambdaValues,workingData3, 'c-.' )
    print ("///////////////////////////////////////////////////////////////")
...
unittestNormalizeSpectra("testNormalisation7.sm")
